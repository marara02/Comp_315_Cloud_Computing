...
class Data_Processing {
    ...
    format_date_birth(db) {
        ...
        let dbirth_format = dbirth_clean.toLocaleDateString('en-GB', {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit'
        })
    ...
    percentage_altered() {
        var total = (this.formatted_user_data).length;
        var altered = 0;

        for (let i = 0; i < total; i++) {
            if (JSON.stringify(this.formatted_user_data[i]) !== JSON.stringify(this.cleaned_user_data[i])) {
                altered++;
            }
        }
